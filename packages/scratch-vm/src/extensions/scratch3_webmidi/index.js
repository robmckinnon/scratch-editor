...
let mCC_change_event = false;
let mMIDI_event		= false;
let mKey_on_event	= false;
let mKey_off_event	= false;
let mPBend_event	= false;
let mPC_event		= false;

let mCC_change_flag	= false;
let mKey_on_flag 	= false;
let mKey_off_flag 	= false;
let mPC_flag		= false;
let mPBend_flag		= false;
...
*/
    if (mOutputs != null){
        if (mOutDev < mOutputs.length){
            const l_output = mOutputs[mOutDev];
            if (l_output != null){
                l_output.send([data1, data2, data3], 0);
            }
        }

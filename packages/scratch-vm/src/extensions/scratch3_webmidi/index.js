...
let mOutDev = 0; // Output Device Number

const mCtlbuf = new Array(0x80);
const mNoteOn = new Array(0x80);
const mNoteOff = new Array(0x80);

let mCcChangeEvent = false;
let mMidiEvent = false;
let mKeyOnEvent = false;
let mKeyOffEvent = false;
let mPBendEvent = false;
let mPcEvent = false;

let mCcChangeFlag = false;
let mKeyOnFlag = false;
let mKeyOffFlag = false;
let mPcFlag = false;
let mPBendFlag = false;
...
const mNoteon = function (note, vel) {
    mNoteNum = note;
    mNoteVel = vel;
...
const mMidiin = function (event){ /* MIDI parse */
// console.log(event.data[0]);
    switch (event.data[0] & 0xF0){
    case 0x80:
        mMidiEvent = true;
        mNoteon(event.data[1], 0);
        break;
    case 0x90:
        mMidiEvent = true;
        mNoteon(event.data[1], event.data[2]);
        break;
    case 0xA0:
        break;
    case 0xB0:
...
const success = function (midiAccess) {
    mMIDI = midiAccess;
    let msg = 'Success MIDI!\n';
    let inum = 0; let onum = 0;
    ...
    if (typeof mMIDI.inputs === 'function') {
        ...
        msg += 'input Device ';
        const inputIterator = mMIDI.inputs.values();
        mInputs = [];
        ...
        msg += 'Output Device ';
        const outputIterator = mMIDI.outputs.values();
        mOutputs = [];
    ...
    for (let i = 0; i < mInputs.length; i++){
        mInputs[i].onmidimessage = mMidiin;
    }
    //  alert( "Success MIDI!\n", msg );
...
const mMidiout = function (event, note, vel){
    const data1 = event & 0xFF;
    const data2 = note & 0x7F;
    const data3 = vel & 0x7F;

    /* if(mOutputs!=null){
  for(const i=0; i<mOutputs.length; i++){
   var lOutput=mOutputs[i];
   if(lOutput!=null){
    lOutput.send([data1,data2,data3], 0);
   }
  }
 }
*/
    if (mOutputs !== null){
        if (mOutDev < mOutputs.length){
            const lOutput = mOutputs[mOutDev];
            if (lOutput !== null){
                lOutput.send([data1, data2, data3], 0);
            }
        }
...
const mMidiout2byte = function (event, data){
    const data1 = event & 0xFF;
    const data2 = data & 0x7F;

    /* if(mOutputs!=null){
  for(const i=0; i<mOutputs.length; i++){
   var lOutput=mOutputs[i];
   if(lOutput!=null){
    lOutput.send([data1,data2], 0);
   }
  }
 }
*/
...
const mSysexout = function (data, size){
...
const sendstring = function (text) {
    let i; const j = text.length; let k = 0;
    let str = null;
    let ss = null;
    mDex = new Array(j);
    ...
    for (i = 0; i < j - 1; i++){
        ...
        if ((ss = mTextMap[str]) !== null){
            mDex[k++] = ss;
            i++;
        } else if ((ss = mTextMap[text[i]]) !== null){
            mDex[k++] = ss;
        }
    }
    if (i < j){
...
class Scratch3WebMIDI {
 ...
 success(midiAccess)
 {
  ...
  for(const i=0; i<mInputs.length;i++){
   this.mInputs[i].onmidimessage=this.mMidiin;
  }
  alert( "Success MIDI!" );
 }
 ...
 */
    ...
    s_Getcc () {
        // Reset alarm_went_off if it is true, and return true
        // otherwise, return false.
        if (mCcChangeEvent === true) {
            mCcChangeEvent = false;
            return true;
        }
        return false;
    }
    ...
    // NOTE ON
    s_Noteon_out (args){
        const chnum = (args.channelnum & 0x0F) - 1;
        return mMidiout(0x90 + chnum, args.notenum & 0x7F, args.velo & 0x7F);
    }

    s_Noteon_out_duration (args){
        const chnum = (args.channelnum & 0x0F) - 1;
        let irticks = args.duration;
        if (irticks < 10) irticks = 10;
        const waittime = irticks / mResolution * 60 / mTempo * 1000;
        setTimeout(() => {
            mMidiout(0x80 + chnum, args.notenum & 0x7F, 0x40);
        }
        , waittime);
        return mMidiout(0x90 + chnum, args.notenum & 0x7F, args.velo & 0x7F);
    }
    ...
    s_Noteoff_out (args){
        const chnum = (args.channelnum & 0x0F) - 1;
        return mMidiout(0x80 + chnum, args.notenum & 0x7F, args.velo & 0x7F);
    }

    // Program Change
    s_ProgramChange (args){
        const chnum = (args.channelnum & 0x0F) - 1;
        const pgnum = (args.pnumber & 0x7F);
        return mMidiout2byte(0xC0 + chnum, pgnum);
    }
...
*/
    ...
    s_RestTicks (args, util) {
        // const iCount = this.mCount;

        if (this._stackTimerNeedsInit(util)) {
            const irticks = args.rticks;
            const waittime = irticks / mResolution * 60 / mTempo;
...
const initTextanlz = function () {
    mTextMap = new Array(textMapANum + textMapBNum);
    for (let i = 0; i < textMapANum; i++){
        mTextMap[textMapA[i][0]] = textMapA[i][1];
    }
    for (let i = 0; i < textMapBNum; i++){
